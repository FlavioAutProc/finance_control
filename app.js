...
class FinanceApp {
  ...
  checkAndUpdateCache() {
    setInterval(async () => {
      if (this.cache.lastUpdate && Date.now() - this.cache.lastUpdate > 60000) {
        // 1 minuto
    ...
    try {
      ...
      if (percentageElement && generalLimit) {
        const percentage = Math.min(
          (monthlyExpenses / generalLimit) * 100,
          100
        );
  ...
  initDatabase() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        console.log(
          `🔄 Atualizando banco de dados: v${oldVersion} → v${newVersion}`
        );
        ...
        if (oldVersion < 2) {
          ...
          if (!transactionStore.indexNames.contains("transactionHash")) {
            transactionStore.createIndex("transactionHash", "transactionHash", {
              unique: false,
            });
          }
        }
      ...
      const defaultSettings = [
        {
          ...
          value: ["Despesa", "Receita", "Transferência"],
        },
        {
          key: "categories",
          value: [
  ...
  bindEvents() {
    try {
      ...
      openCalendarBtn.innerHTML =
        '<i class="fas fa-calendar-alt"></i> <span>Selecionar Datas</span>';
      ...
      // Salvar transação - VALIDAÇÃO MELHORADA
      ...
      if (transactionForm) {
        transactionForm.addEventListener("submit", async (e) => {
          ...
          try {
            ...
            // Liberar o lock após 1.5 segundos
            setTimeout(() => {
              this.transactionLock = false;
            }, 1500);
          }
      ...
      const editGeneralLimitBtn = document.querySelector(
        '[data-action="edit-general-limit"]'
      );
      if (editGeneralLimitBtn) {
        editGeneralLimitBtn.addEventListener("click", async () => {
          ...
          const newLimit =
            parseFloat(
              prompt("Digite o limite geral mensal (R$):", currentLimit || "")
            ) || 0;
          if (newLimit > 0) {
  ...
  // Adicionar este método na classe FinanceApp (em qualquer lugar apropriado)
  async loadTransactionModalData(transaction) {
    try {
      console.log("🔄 Carregando dados para modal de edição...");
      ...
      if (typeSelect) {
        ...
        types.forEach((type) => {
          ...
          if (transaction.type === type) option.selected = true;
          typeSelect.appendChild(option);
        });
      }

      // Carregar categorias baseado no tipo selecionado
      await this.updateModalCategoriesBasedOnType();
      ...
      if (categorySelect) {
        // Usar setTimeout para garantir que as opções foram carregadas
        setTimeout(() => {
          categorySelect.value = transaction.category;

          // Se não encontrar a categoria, adicionar como opção
      ...
      const classificationSelect = document.getElementById(
        "modal-transaction-classification"
      );
      if (classificationSelect) {
        ...
        classifications.forEach((cls) => {
          ...
          if (transaction.classification === cls) option.selected = true;
          classificationSelect.appendChild(option);
        });

        // Se não encontrar a classificação, manter valor
        if (!classificationSelect.value && transaction.classification) {
          const option = document.createElement("option");
          option.value = transaction.classification;
          option.textContent = transaction.classification;
          option.selected = true;
          classificationSelect.appendChild(option);
        }
      }

      console.log("✅ Dados do modal carregados com sucesso");
    } catch (error) {
  ...
  initDateFilters() {
    try {
      ...
      if (investmentDateInput)
        investmentDateInput.value = now.toISOString().split("T")[0];
    } catch (error) {
  ...
  validateTransactionForm() {
    try {
      ...
      const requiredFields = [
        { id: "transaction-date", name: "Data" },
        { id: "transaction-type", name: "Tipo" },
        { id: "transaction-category", name: "Categoria" },
        { id: "transaction-value", name: "Valor" },
        { id: "transaction-description", name: "Descrição" },
      ];
  ...
  async loadTransactionTypes() {
    try {
      const types = (await this.getSetting("transactionTypes")) || [];
      const selects = ["transaction-type", "filter-type", "category-type"];
  ...
  async loadCategoriesForLimits() {
    try {
      ...
      console.log(
        "✅ Categorias para limites carregadas:",
  ...
  async updateTransactionsTable(transactions, total) {
    try {
      ...
      transactions.forEach((transaction) => {
        ...
        // Formatar data - garantir que sempre tenha uma data
        ...
        try {
          ...
          console.warn("Erro ao formatar data:", error, transaction);
        }

        // Tipo - garantir que tenha valor
        const type = transaction.type || "Desconhecido";

        // Categoria - garantir que tenha valor
        const category = transaction.category || "Não categorizada";

        // Valor - garantir que tenha valor e seja número
        let value = "0,00";
        try {
          const numValue = parseFloat(transaction.value) || 0;
          value = numValue.toFixed(2).replace(".", ",");
        } catch (error) {
          console.warn("Erro ao formatar valor:", error, transaction);
        }

        // Descrição - garantir que tenha valor
        const description = transaction.description || "Sem descrição";

        // Status - garantir lógica correta
        let status = "";
        let statusClass = "";
        ...
        try {
          ...
          console.warn("Erro ao determinar status:", error, transaction);
          status = "Pendente";
          statusClass = "warning";
        }

        // Construir a linha da tabela com os dados garantidos
        row.innerHTML = `
                <td>${formattedDate}</td>
                <td><span class="badge ${
                  type === "Receita" ? "success" : "danger"
                }">${type}</span></td>
                <td>${category}</td>
      ...
      console.log(
        `✅ Tabela atualizada: ${transactions.length} transações exibidas`
      );
    } catch (error) {
  ...
  updateTransactionsSummary(transactions) {
    try {
      ...
      const overdue = transactions.filter(
        (t) => t.status !== "paid" && new Date(t.date) < new Date()
      ).length;

      const currencySymbol =
        document.querySelector("#currency-symbol")?.value || "R$";
      ...
      updateElement(
        ...
        `${currencySymbol} ${totalValue.toFixed(2).replace(".", ",")}`
      );
  ...
  updatePagination(totalPages) {
    try {
      ...
      if (nextBtn) {
        nextBtn.disabled =
          this.transactionsPage === totalPages || totalPages === 0;
      }
    } catch (error) {
  ...
  async loadInvestments() {
    try {
      ...
      investments.forEach((investment) => {
        ...
        const profitability =
          investment.amount > 0 ? (profit / investment.amount) * 100 : 0;
        ...
        row.innerHTML = `
                    ...
                    <td>${formattedDate}</td>
                    <td><strong>${currencySymbol} ${investment.amount
          .toFixed(2)
          .replace(".", ",")}</strong></td>
                    <td><strong>${currencySymbol} ${investment.currentValue
          .toFixed(2)
          .replace(".", ",")}</strong></td>
                    <td>
                        <span class="badge ${
                          profitability >= 0 ? "success" : "danger"
                        }">
                            ${
                              profitability >= 0 ? "+" : ""
  ...
  updateInvestmentSummary(investments, totalInvested = 0, totalCurrent = 0) {
    try {
      ...
      updateElement(
        "total-invested",
        `${currencySymbol} ${totalInvested.toFixed(2).replace(".", ",")}`
      );
      updateElement(
        "current-value",
        `${currencySymbol} ${totalCurrent.toFixed(2).replace(".", ",")}`
      );
      updateElement(
        "total-profitability",
  ...
  async updateGeneralLimit(generalLimit) {
    try {
      ...
      if (generalLimit) {
        ...
        currentElement.textContent = `R$ ${monthlyExpenses
          .toFixed(2)
          .replace(".", ",")}`;
        ...
        if (editBtn) {
          editBtn.addEventListener("click", async (e) => {
            ...
            const newLimit =
              parseFloat(
                prompt("Digite o novo limite geral mensal (R$):", generalLimit)
              ) || 0;
            if (newLimit > 0) {
              await this.saveSetting("generalLimit", newLimit);
              await this.loadLimits();
        ...
        if (addBtn) {
          addBtn.addEventListener("click", async (e) => {
            e.stopPropagation();
            const newLimit =
              parseFloat(prompt("Digite o limite geral mensal (R$):", "")) || 0;
            if (newLimit > 0) {
              await this.saveSetting("generalLimit", newLimit);
              await this.loadLimits();
              this.showAlert(
                "Limite geral configurado com sucesso!",
                "success"
              );
            }
          });
    ...
    try {
      ...
      expenseCategories.forEach((category) => {
        ...
        if (limit) {
          ...
          const percentage =
            limit.amount > 0 ? (spent / limit.amount) * 100 : 0;
          ...
          row.innerHTML = `
                        <td>${categoryName}</td>
                        <td><strong>R$ ${limit.amount
                          .toFixed(2)
                          .replace(".", ",")}</strong></td>
                        <td>R$ ${spent
                          .toFixed(2)
                          .replace(".", ",")} (${percentage.toFixed(1)}%)</td>
                        <td><span class="badge ${statusClass}">${status}</span></td>
                        <td>
                            <div class="table-actions">
                                <button class="action-btn edit" data-id="${
      ...
      console.log(
        `✅ Limites carregados: ${limits.length} configurados, ${expenseCategories.length} categorias totais`
      );
    } catch (error) {
      ...
      if (generalLimit && monthlyExpenses > generalLimit) {
        ...
        alert.innerHTML = `
                    ...
                    <div>
                        ...
                        <p>Você gastou R$ ${monthlyExpenses
                          .toFixed(2)
                          .replace(".", ",")} de R$ ${generalLimit
          .toFixed(2)
          .replace(".", ",")} este mês.</p>
                    </div>
                `;
        ...
        if (spent > limit.amount) {
          ...
          alert.innerHTML = `
                        ...
                        <div>
                            ...
                            <p>Você gastou R$ ${spent
                              .toFixed(2)
                              .replace(".", ",")} de R$ ${limit.amount
            .toFixed(2)
            .replace(".", ",")}.</p>
                        </div>
                    `;
  ...
  openTransactionModal(transaction = null) {
    try {
      ...
      if (!modal || !modalTitle) {
        console.warn("Modal não encontrado, usando fallback para seção");
        this.switchSection("new-transaction");
        if (transaction) {
      ...
      if (transaction) {
        ...
        if (modalBody) {
          modalBody.innerHTML = this.createTransactionFormHTML(transaction);
        }
        ...
        // Carregar dados nos selects APÓS o modal estar visível
        setTimeout(async () => {
          await this.loadTransactionModalData(transaction);

          // Adicionar campo oculto para identificar edição
          const form = modal.querySelector("#transaction-form");
          if (form) {
            let editingInput = form.querySelector('[name="editing"]');
            if (!editingInput) {
              editingInput = document.createElement("input");
              editingInput.type = "hidden";
              editingInput.name = "editing";
            ...
            // Vincular eventos ao formulário no modal
            this.bindTransactionModalEvents();
          }
        }, 50);
      } else {
        // Modo criação - ir para a seção de nova transação
        this.switchSection("new-transaction");
        this.clearTransactionForm();
      }
    } catch (error) {
      console.error("Erro ao abrir modal de transação:", error);
      // Fallback: ir para a seção de nova transação
      this.switchSection("new-transaction");
      if (transaction) {
        this.populateTransactionForm(transaction);
      }
  ...
  // Adicionar método para criar HTML do formulário no modal
  createTransactionFormHTML(transaction) {
    return `
                <form id="transaction-form">
                    <div class="form-row">
                        <div class="form-group">
                            <label for="modal-transaction-date">
                                <i class="fas fa-calendar"></i> Data de Vencimento *
                            </label>
                            <input type="date" id="modal-transaction-date" value="${
                              transaction.date
                            }" required>
                        </div>
                        <div class="form-group">
                            <label for="modal-transaction-type">
                        ...
                        <div class="form-group">
                            <label for="modal-transaction-category">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="modal-transaction-value">
                                <i class="fas fa-money-bill-wave"></i> Valor (R$) *
                            </label>
                            <input type="number" id="modal-transaction-value" 
                                value="${
                                  transaction.value
                                }" step="0.01" min="0" required>
                        </div>
                        <div class="form-group">
                            <label for="modal-transaction-subcategory">
                                <i class="fas fa-tags"></i> Subcategoria
                            </label>
                            <input type="text" id="modal-transaction-subcategory" 
                                value="${transaction.subcategory || ""}">
                        </div>
                        <div class="form-group">
                            <label for="modal-transaction-classification">
                                <i class="fas fa-star"></i> Classificação
                            </label>
                            <select id="modal-transaction-classification">
                                <!-- Classificações serão carregadas dinamicamente -->
                            </select>
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label for="modal-transaction-description">
                            <i class="fas fa-align-left"></i> Descrição Detalhada *
                        </label>
                        <textarea id="modal-transaction-description" rows="3" required>${
                          transaction.description
                        }</textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="modal-transaction-notes">
                            <i class="fas fa-sticky-note"></i> Observações
                        </label>
                        <textarea id="modal-transaction-notes" rows="2">${
                          transaction.notes || ""
                        }</textarea>
                    </div>
  ...
  // Método para vincular eventos no modal
  bindTransactionModalEvents() {
    const form = document.getElementById("transaction-form");
    if (form) {
      // Remover listener antigo para evitar duplicação
      form.removeEventListener("submit", this.handleModalFormSubmit);

      // Adicionar novo listener
      this.handleModalFormSubmit = (e) => {
        e.preventDefault();
        this.saveTransactionFromModal();
      };

      form.addEventListener("submit", this.handleModalFormSubmit);

      // Vincular evento ao tipo para atualizar categorias
      const typeSelect = document.getElementById("modal-transaction-type");
      if (typeSelect) {
        typeSelect.addEventListener("change", () => {
          this.updateModalCategoriesBasedOnType();
        });
      }
    }
  }
  async saveTransactionFromModal() {
    try {
      ...
      const editingInput = form.querySelector('[name="editing"]');
      const isEditing = editingInput && editingInput.value;

      // Preparar transação
      const transaction = {
        date: document.getElementById("modal-transaction-date").value,
        type: document.getElementById("modal-transaction-type").value,
        category: document.getElementById("modal-transaction-category").value,
        classification:
          document.getElementById("modal-transaction-classification").value ||
          null,
        value: parseFloat(
          document.getElementById("modal-transaction-value").value
        ),
        subcategory:
          document.getElementById("modal-transaction-subcategory").value ||
          null,
        description: document.getElementById("modal-transaction-description")
          .value,
        notes: document.getElementById("modal-transaction-notes").value || null,
        status: "pending",
      ...
      if (isEditing) {
        ...
        const existingId = parseInt(editingInput.value);
        const existingTransaction = await this.getTransaction(existingId);

        if (existingTransaction) {
          transaction.id = existingTransaction.id;
          transaction.createdAt = existingTransaction.createdAt;
          transaction.status = existingTransaction.status; // Manter status atual
      ...
      console.error("Erro ao salvar transação do modal:", error);
      this.showAlert("Erro ao salvar transação", "danger");
    }
  }

  // Método para salvar transação do modal

  // Método para atualizar categorias no modal
  async updateModalCategoriesBasedOnType() {
    try {
      ...
      console.error("Erro ao atualizar categorias no modal:", error);
    }
  }

  // Método para preencher o formulário na seção new-transaction (fallback)
  async populateTransactionForm(transaction) {
    try {
      document.getElementById("transaction-date").value = transaction.date;
      document.getElementById("transaction-type").value = transaction.type;

      await this.updateCategoriesBasedOnType();

      // Esperar um pouco para as categorias carregarem
      setTimeout(() => {
        document.getElementById("transaction-category").value =
          transaction.category;
        document.getElementById("transaction-classification").value =
          transaction.classification || "";
        ...
        // Adicionar campo oculto para edição
        let editingInput = document.querySelector(
          '#transaction-form [name="editing"]'
        );
        if (!editingInput) {
          editingInput = document.createElement("input");
  ...
  async saveTransaction() {
    try {
      ...
      const form = document.getElementById("transaction-form");
      if (!form) {
        this.showAlert("Formulário não encontrado", "danger");
        return;
      }
      ...
      if (!dateInput || !dateInput.value) {
        this.showAlert("Informe a data da transação", "warning");
        this.transactionLock = false;
        return;
      }

      if (!typeInput || !typeInput.value) {
        this.showAlert("Selecione o tipo da transação", "warning");
        this.transactionLock = false;
        return;
      }

      if (!categoryInput || !categoryInput.value) {
        this.showAlert("Selecione a categoria da transação", "warning");
        this.transactionLock = false;
        return;
      }
      ...
      if (
        ...
        this.showAlert("Informe um valor válido maior que zero", "warning");
        this.transactionLock = false;
        return;
      }

      if (!descriptionInput || !descriptionInput.value.trim()) {
        this.showAlert("Informe uma descrição para a transação", "warning");
        this.transactionLock = false;
        return;
      }

      // Preparar objeto da transação
      const transaction = {
        date: dateInput.value,
        type: typeInput.value,
        category: categoryInput.value,
        classification: classificationInput.value || null,
        value: parseFloat(valueInput.value.replace(",", ".")) || 0,
        subcategory: subcategoryInput ? subcategoryInput.value || null : null,
      ...
      console.log("📝 Dados da transação preparados:", transaction);
      ...
      // Verificar duplicata (apenas para novas transações)
      if (!isEditing) {
      ...
      let result;
      if (isEditing) {
        // Modo edição: obter transação existente e atualizar
        const existingId = parseInt(editingInput.value);
        const existingTransaction = await this.getTransaction(existingId);

        if (!existingTransaction) {
          this.showAlert("Transação não encontrada para edição", "warning");
          this.transactionLock = false;
          return;
        }

        // Preservar dados que não devem mudar
        transaction.id = existingTransaction.id;
        transaction.createdAt = existingTransaction.createdAt;
        transaction.status = existingTransaction.status; // Manter status atual
      ...
      // Se estivermos na seção de nova transação, voltar para dashboard
      if (this.currentSection === "new-transaction" && !isEditing) {
  ...
  async saveInvestment() {
    try {
      ...
      const investment = {
        ...
        description:
          document.getElementById("investment-description").value || null,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
  ...
  async saveLimit() {
    try {
      ...
      this.showAlert(
        `Limite ${isEditing ? "atualizado" : "salvo"} com sucesso!`,
        "success"
      );
  ...
  async loadSettingsUI() {
    try {
      ...
      if (autoCategorizeCheckbox)
        autoCategorizeCheckbox.checked = autoCategorize;
  ...
  async editCategory(oldName, oldType, newName) {
    try {
      ...
      const index = categories.findIndex(
        (cat) => cat.name === oldName && cat.type === oldType
      );
      ...
      if (
        categories.some((cat) => cat.name === newName && cat.type === oldType)
      ) {
        this.showAlert("Esta categoria já existe", "warning");
        return;
      }
  ...
  async updateTransactionsCategory(oldCategory, newCategory) {
    try {
      ...
      if (updatedTransactions.length > 0) {
        console.log(
          `🔄 ${updatedTransactions.length} transações atualizadas com nova categoria`
        );
        // Limpar cache
        this.cache.transactions = null;
        await this.refreshCache();
        ...
        datasets: [
          {
            ...
            borderColor: ["rgb(76, 201, 240)", "rgb(247, 37, 133)"],
            borderWidth: 2,
          },
        ],
      };
  ...
  async createTrendChart(transactions) {
    try {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        trendLabels.push(
          `${monthName}/${date.getFullYear().toString().slice(2)}`
        );
  ...
  async payAllMonthlyTransactions() {
    try {
      ...
      if (monthlyTransactions.length === 0) {
        this.showAlert(
          "Não há transações pendentes para pagar este mês",
          "info"
        );
        return;
      }
      ...
      if (
        confirm(
          `Deseja marcar todas as ${monthlyTransactions.length} transações pendentes como pagas?`
        )
      ) {
        ...
        this.showAlert(
          `${monthlyTransactions.length} transações marcadas como pagas!`,
          "success"
        );
        await this.refreshCache();
        this.loadTransactions();
  ...
  async exportData() {
    try {
      ...
      const period =
        document.getElementById("export-period")?.value || "current-month";
      const dataType =
        document.getElementById("export-data-type")?.value || "transactions";
        ...
        if (dataType === "investments") {
          data = filtered.map((inv) => ({
            ...
            Rentabilidade:
              inv.amount > 0
                ? (
                    ((inv.currentValue - inv.amount) / inv.amount) *
                    100
                  ).toFixed(2) + "%"
                : "0%",
  ...
  exportToPDF(data, filename) {
    try {
      ...
      // Tabela
      const headers = [
        ["Data", "Tipo", "Categoria", "Valor", "Descrição", "Situação"],
      ];
      const rows = data.map((item) => [
        ...
        item.Categoria,
        `R$ ${item.Valor}`,
        item.Descrição.substring(0, 20) +
          (item.Descrição.length > 20 ? "..." : ""),
        item.Situação,
      ]);
  ...
  async generatePDFReport() {
    try {
      ...
      const period =
        document.getElementById("pdf-period")?.value || "current-month";
      const reportType =
        document.getElementById("pdf-report-type")?.value || "complete";
      ...
      doc.setFontSize(11);
      doc.text(
        `Receitas: R$ ${monthlyIncome.toFixed(2).replace(".", ",")}`,
        14,
        65
      );
      doc.text(
        `Despesas: R$ ${monthlyExpense.toFixed(2).replace(".", ",")}`,
        14,
        73
      );
      doc.text(
        `Saldo: R$ ${(monthlyIncome - monthlyExpense)
          .toFixed(2)
          .replace(".", ",")}`,
        14,
        81
      ...
      doc.text(
        `Investimentos: R$ ${totalInvestment.toFixed(2).replace(".", ",")}`,
        14,
        89
      );
      ...
      const recommendations = [
        `- A categoria "${categoryTotals[0]?.category || "N/A"}" representa ${
          categoryTotals[0]
            ? ((categoryTotals[0].total / totalExpenses) * 100).toFixed(1)
            : "0"
        }% dos seus gastos.`,
  ...
  parseInvestmentRow(row) {
    try {
      ...
      return {
        ...
        currentValue: parseFloat(
          (row["Valor Atual"] || row.currentValue || amount)
            .toString()
            .replace(",", ".")
        ),
...
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    alert(
      "Erro ao inicializar o sistema financeiro. Por favor, recarregue a página."
    );
  }
});
